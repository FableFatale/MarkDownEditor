...
- Name: 测试页面功能测试 >> 测试页面编辑器功能
- Location: D:\MarkDownEditor\e2e\test-page.spec.ts:126:3

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: getByText('预览区域') resolved to 5 elements:
    1) <div class="cm-line">切换到深色模式，预览区域应该也变成深色背景。</div> aka getByRole('textbox').getByText('切换到深色模式，预览区域应该也变成深色背景。')
    2) <div class="cm-line">| 深色模式 | ✅ | 预览区域正确响应主题 |</div> aka getByText('| 深色模式 | ✅ | 预览区域正确响应主题 |')
    3) <span>预览区域</span> aka getByText('预览区域', { exact: true })
    4) <p>切换到深色模式，预览区域应该也变成深色背景。</p> aka getByRole('paragraph').filter({ hasText: '切换到深色模式，预览区域应该也变成深色背景。' })
    5) <td class="px-3 py-2 border-t border-gray-200 dark:border-gray-700 text-sm">预览区域正确响应主题</td> aka getByRole('cell', { name: '预览区域正确响应主题' })

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText('预览区域')

    at D:\MarkDownEditor\e2e\test-page.spec.ts:128:42
```
