...
```
Error: expect.toBeVisible: Error: strict mode violation: getByText('预览区域') resolved to 3 elements:
    1) <span>预览区域</span> aka getByText('预览区域', { exact: true })
    2) <p>切换到深色模式，预览区域应该也变成深色背景。</p> aka getByText('切换到深色模式，预览区域应该也变成深色背景。')
    3) <td class="px-3 py-2 border-t border-gray-200 dark:border-gray-700 text-sm">预览区域正确响应主题</td> aka getByRole('cell', { name: '预览区域正确响应主题' })
