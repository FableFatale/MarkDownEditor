...
- Name: 主程序功能测试 >> 字数统计应该显示正确信息
- Location: D:\MarkDownEditor\e2e\main-app.spec.ts:146:3

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: getByText(/字数/) resolved to 2 elements:
    1) <span class="ͼ9">字数统计</span> aka getByRole('textbox').getByText('字数统计')
    2) <strong>字数统计</strong> aka getByRole('strong').filter({ hasText: '字数统计' })

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText(/字数/)

    at D:\MarkDownEditor\e2e\main-app.spec.ts:148:40
```
